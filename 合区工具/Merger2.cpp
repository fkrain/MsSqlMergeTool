...
int main(int argc, char *argv[]) {
	...
	try {
		...
		LogChanged(fAC, "<info>\t处理LoginDB，复制帐号开始...\n");
		...
		while( !rs->GetRSEOF() ) {
			...
			if( !bEof ) { // 目标数据库存在改帐号了。进行账号改名
				...
				sprintf(msg,"<record>\t 原始玩家账号:[%s] 修正玩家账号:[%s]\n", old_playeraccount, new_playeraccount);
				Log(msg);
				sprintf(msg,"%s,%s\n", old_playeraccount, new_playeraccount);
				Log(msg);
			}
	...
	try {
		...
		LogChanged(fPNC, "<info>\t载入目的数据库CSL_SETUP表值\n");
		// 初始Des的ID_SETUP
		...
		while(!rs->GetRSEOF()) {
			...
			try {
				...
				// 仅存在Player_quest_ex里。
				if(!new_playerid) {
					sprintf(msg,"<warrning>\t角色ID[%d]仅存在CSL_PLAYER_QUEST_EX中\n",old_playerid);
					Log(msg);
					rs->MoveNext();
					continue;
				}
